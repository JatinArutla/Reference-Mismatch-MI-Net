...
import numpy as np
import pandas as pd
from refshift.experiments.common import FAMILIES, load_dataset_yaml, load_graphs, get_subject_data
...
def run_lofo(repo_root: Path, dataset_id: str, cache_root: str, out_dir: str, cfg: TrainConfig, subjects=None):
    ...
    save_csv(df.groupby('heldout_family', as_index=False)['acc'].mean(), out/'metrics_summary.csv')
