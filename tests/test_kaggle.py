"""Tests for refshift.kaggle module integrity.
...
``def _patch_moabb_dreyer_no_unzip`` line from refshift.kaggle, leaving
...
def test_patch_moabb_dreyer_no_unzip_is_defined():
    ...
    from refshift.kaggle import _patch_moabb_dreyer_no_unzip
...
def test_all_dataset_setup_helpers_defined():
    ...
    for name in expected_helpers:
        assert hasattr(kaggle, name), f"Missing function: refshift.kaggle.{name}"
        assert callable(getattr(kaggle, name)), f"Not callable: refshift.kaggle.{name}"
